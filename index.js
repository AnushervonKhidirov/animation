...
window.addEventListener(
  ...
  () => {
    ...
    const stepsAnimationDuration =
      parseFloat(window.getComputedStyle(animationWrapper).getPropertyValue('--step-animation-duration')) * 1000;
    const stepsAnimationDelay =
      parseFloat(window.getComputedStyle(animationWrapper).getPropertyValue('--step-animation-delay')) * 1000;
    const ctaNextStepDelay =
      stepsAnimationDelay - stepsAnimationDuration >= 0 ? stepsAnimationDelay - stepsAnimationDuration : 0;
    ...
    function showFrame(currFrame) {
      ...
      const steps = frame.querySelectorAll('[class*="step-"]');
      const lastStep = getLastAnimationStep(steps);
      if (!lastStep) return;

      lastStep.currFrame = currFrame;
      ...
      if (showCtaFirst) {
        lastStep.addEventListener('animationend', nextFrameEventDispatch);
      } else {
        animationWrapper.dispatchEvent(startAnimationEvent);
        lastStep.addEventListener('animationend', showCtaEventDispatch);
      }
    }
    ...
    function getLastAnimationStep(steps) {
      let lastStepClass = '';
      let lastStep = null;

      steps.forEach(step => {
        const classNames = step.getAttribute('class').split(' ');
        const className = classNames.find(className => /^step-\d+$/.test(className));

        if (className > lastStepClass) {
          lastStepClass = className;
          lastStep = step;
        }
      });
