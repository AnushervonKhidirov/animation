class BannerAnimation {
    ...
    eventsHandler() {
        this.parentElem.addEventListener(this.initEventName, e => {
            ...
            sendMessage(this.getEventDetails(e));
        });
        ...
        this.parentElem.addEventListener(this.frameSwitchedEventName, e => {
            if (this.developerMode) {
        ...
        this.parentElem.addEventListener(this.frameInnerAnimEndEventName, e => {
            if (this.developerMode) {
            ...
            const { nextFrameDelay } = this.getFrameData(this.frames[this.currFrameIndex]);

            if (shouldSwitchFrame) {
                setTimeout(this.nextFrame.bind(this), nextFrameDelay ?? this.nextFrameDelay);
            }

            sendMessage(this.getEventDetails(e));
        });

        this.parentElem.addEventListener(this.stepAnimEndEventName, e => {
            if (this.developerMode) {
        ...
        this.parentElem.addEventListener(this.ctaAnimEndEventName, e => {
            if (this.developerMode) {
    ...
    getFrameData(frame) {
        const nextFrameDelay = frame.getAttribute(this.dataNextFrameDelay);

        return {
            logoTitle: frame.getAttribute(this.dataLogo),
    ...
    getEventDetails(e) {
        ...
        return {
            ...
            nextFrameDelay: frameData.nextFrameDelay ?? this.nextFrameDelay,
            showCtaFirst: this.showCtaFirst,
            loopRotate: this.loopRotate,
        };
    }
